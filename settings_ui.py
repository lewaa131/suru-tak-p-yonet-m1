...
def open_cycle_settings(app):
    ...
    intro=card(body,'Döngü hazırlığı','Tercihler kaydedilir; otomatik takip sonraki adımda açılacak.')
    # Existing working controls remain available without applying any draft.
    intro.add_widget(big_button('Şu an çalışan ayarlar',lambda *_:app.active_farm_options(),48,'secondary'))
    inputs={}
    toggles={}
    for key,title,description,default,low,high,toggle in DAY_SETTINGS:
        ...
        if toggle:
            ...
            row.add_widget(Label(text='Hatırlatma tercihi',color=INK,font_size=dp(13),halign='left'))
            switch=GreenSwitch(active=values[toggle])
            toggles[toggle]=switch
            row.add_widget(switch)
            box.add_widget(row)
    box=card(body,'Süt kaydı','Süt miktarının ayrı sağım veya günlük toplam olarak girişini seç.')
    milk=Choices([('separate','Sabah / akşam'),('daily','Günlük toplam')],values['milk_mode'])
    box.add_widget(milk)
    box=card(body,'Takvim','Görev takviminin açılışta göstereceği zaman aralığını seç.')
    ...
    def save(*_):
        draft={key:entry.text for key,entry in inputs.items()}
        ...
        try: save_cycle(path,draft)
        except (ValueError,OSError) as exc:
            error.text=str(exc); return
        popup.dismiss()
        app.notice('Tercihler kaydedildi.\nOtomatik döngü henüz açık değil.')
