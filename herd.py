...
class Herd:
    ...
    def all(self):
        return [dict(row) for row in self.db.execute('SELECT * FROM cows ORDER BY tag')]
    ...
    def record_birth(self, cow_id, day, today=None):
        cow = self.get(cow_id)
        ...
        with self.db:
            self.db.execute("UPDATE cows SET pregnant=0,insemination='',state='Sağmal' WHERE id=?", (cow_id,))
            self.db.execute('INSERT INTO events(cow_id,kind,day) VALUES (?,?,?)', (cow_id,'Doğum yaptı',day.isoformat()))

    def register(self, info, gestation_days=283):
