...
class MetricCard(ButtonBehavior, BoxLayout):
    """Large, readable counters with a distinct colored icon badge."""
    def __init__(self,title,value,kind,tone,action,**kwargs):
        super().__init__(orientation='vertical',padding=dp(14),spacing=dp(4),
                         size_hint_y=None,height=dp(114),**kwargs)
        bg,fg=PALETTES[tone]
        surface(self,bg,22)
        top=BoxLayout(spacing=dp(8))
        self.value=Label(text=str(value),bold=True,color=fg,font_size=sp(30),halign='left')
        self.value.bind(size=lambda w,s:setattr(w,'text_size',s))
        top.add_widget(self.value)
        icon=NavIcon(kind,tint=fg,size_hint_x=None,width=dp(38))
        top.add_widget(icon)
        self.add_widget(top)
        caption=Label(text=title,color=INK,font_size=sp(14),size_hint_y=None,height=dp(24),halign='left')
